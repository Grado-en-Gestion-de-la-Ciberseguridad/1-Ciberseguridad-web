...
cssclasses:
  - hbox
---
[[02. Education/03. OS/00. Linux/01. Workflow/03. Find Files and Directories|Previous]]
[[02. Education/03. OS/00. Linux/01. Workflow/05. Filter Contents|Next]]
...
```shell-session
user@htb[/htb]$ find /etc/ -name shadow
```
...
- **File Descriptors and Redirections**:
```shell-session
user@htb[/htb]$ find /etc/ -name shadow 2>/dev/null
```
...
- **File Descriptors and Redirections**:
```shell-session
user@htb[/htb]$ find /etc/ -name shadow 2>/dev/null > results.txt
```
...
- **File Descriptors and Redirections**:
```shell-session
user@htb[/htb]$ find /etc/ -name shadow 2> stderr.txt 1> stdout.txt
```
...
- **File Descriptors and Redirections**:
```shell-session
user@htb[/htb]$ find /etc/ -name passwd >> stdout.txt 2>/dev/null
```
...
- **File Descriptors and Redirections**:
```shell-session
user@htb[/htb]$ cat << EOF > stream.txt
```
...
- **File Descriptors and Redirections**:
```shell-session
user@htb[/htb]$ find /etc/ -name *.conf 2>/dev/null | grep systemd
```
...
- **File Descriptors and Redirections**:
```shell-session
user@htb[/htb]$ find /etc/ -name *.conf 2>/dev/null | grep systemd | wc -l
```
