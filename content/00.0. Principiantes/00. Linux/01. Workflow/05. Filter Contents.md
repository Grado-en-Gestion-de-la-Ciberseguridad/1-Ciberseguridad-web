...
cssclasses:
  - hbox
---
[[02. Education/03. OS/00. Linux/01. Workflow/04. File Descriptors and Redirections|Previous]]
[[02. Education/03. OS/00. Linux/01. Workflow/06. Regular Expressions|Next]]
...
- **Filter Contents**:
```shell-session
user@htb[/htb]$ cat /etc/passwd | grep -v "false\|nologin" | tr ":" " " | awk '{print $1, $NF}'
...
- **Filter Contents**:
```shell-session
user@htb[/htb]$ cat /etc/passwd | grep -v "false\|nologin" | tr ":" " " | awk '{print $1, $NF}' | sed 's/bin/HTB/g'
...
- **Filter Contents**:
```shell-session
user@htb[/htb]$ cat /etc/passwd | grep -v "false\|nologin" | tr ":" " " | awk '{print $1, $NF}' | wc -l
